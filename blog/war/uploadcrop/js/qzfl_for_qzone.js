(window.constructQZFL = function() {
    ...
    QZFL.returnFn = function(v) {
        ...
        if (window.ActiveXObject || window.msIsStaticHTML) {
            ua.ie = 6; (window.XMLHttpRequest || (agent.indexOf('MSIE 7.0') > -1)) && (ua.ie = 7); (window.XDomainRequest || (agent.indexOf('Trident/4.0') > -1)) && (ua.ie = 8); (agent.indexOf('Trident/5.0') > -1) && (ua.ie = 9); (agent.indexOf('Trident/6.0') > -1) && (ua.ie = 10); (agent.indexOf('Trident/7.0') > -1) && (ua.ie = 11);
    ...
    QZFL.widget = {};
    QZFL.object.map(QZFL.object, QZFL); (function(w) {
    ...
    QZFL.config = QZFL.config || {}; (typeof QZFL.config.debugLevel == 'undefined') && (QZFL.config.debugLevel = 0); (typeof QZFL.config.defaultDataCharacterSet == 'undefined') && (QZFL.config.defaultDataCharacterSet = "GB2312"); (typeof QZFL.config.DCCookieDomain == 'undefined') && (QZFL.config.DCCookieDomain = "qzone.qq.com"); (typeof QZFL.config.domainPrefix == 'undefined') && (QZFL.config.domainPrefix = "qq.com"); (typeof QZFL.config.domain == 'undefined') && (QZFL.config.domain = "qzs.qq.com");
    if (location.host.indexOf("qzone.qq.com") >= 0) { (typeof QZFL.config.domainPrefix == 'undefined') && (QZFL.config.domainPrefix = "qzone.qq.com"); (typeof QZFL.config.domain == 'undefined') && (QZFL.config.domain = "qzs.qzone.qq.com");
    }
    ...
    QZFL.config.FSHelperPage = "/toolpages_fp_gbk.html";
    QZFL.config.defaultShareObject = "/toolpages_getset.swf";
    ...
    QZFL.dom = {
        ...
        searchElementByClassName: function(elem, className) {
            ...
            return QZFL.dom.searchChain(elem, 'parentNode', 
            function(el) {
        ...
        getAncestorBy: function(elem, method) {
            elem = QZFL.dom.get(elem);
            return QZFL.dom.searchChain(elem.parentNode, 'parentNode', 
            function(el) {
        ...
        getFirstChild: function(elem) {
            elem = QZFL.dom.get(elem);
            return elem.firstElementChild || QZFL.dom.searchChain(elem && elem.firstChild, 'nextSibling', 
            function(el) {
        ...
        getLastChild: function(elem) {
            elem = QZFL.dom.get(elem);
            return elem.lastElementChild || QZFL.dom.searchChain(elem && elem.lastChild, 'previousSibling', 
            function(el) {
                return el.nodeType == 1;
            });
        },
        getNextSibling: function(elem) {
            elem = QZFL.dom.get(elem);
            return elem.nextElementSibling || QZFL.dom.searchChain(elem && elem.nextSibling, 'nextSibling', 
            function(el) {
                return el.nodeType == 1;
            });
        },
        getPreviousSibling: function(elem) {
            elem = QZFL.dom.get(elem);
            return elem.previousElementSibling || QZFL.dom.searchChain(elem && elem.previousSibling, 'previousSibling', 
            function(el) {
                return el.nodeType == 1;
            });
        },
        ...
        getElementByTagNameBubble: function(elem, tn) {
            ...
            elem = QZFL.dom.searchChain(elem = QZFL.dom.get(elem), 'parentNode', 
            function(el) {
    ...
    QZFL.event = {
        ...
        delegate: function(delegateDom, selector, eventType, fn, argsArray) {
            var path = "./js/delegate.js?max_age=864000";
            QZFL.imports(path, 
            function() {
    ...
    QZFL.string.timeFormatString = function(date, ptn, baseTime) {
        ...
        if (!ptn) {
            ...
            return ptn.replace(me._re, 
            function(a, b, c) { (rel = b.charAt(0) == '_') && (b = b.charAt(1));
                if (!map[b]) {
                    return a;
                }
                if (!rel) {
    ...
    QZFL.string.StringBuilder.prototype = {
        ...
        rNonWord = /\W/,
        tmpVar,
        rSpeedUp = /^(\w+$)|^\.([\w\-]+$)|^#([\w\-]+$)|^(\w+)\.([\w\-]+$)/; [0, 0].sort(function() {
                        ...
                        if (speedUpMatch[1]) {
                            ...
                            if (context.getElementsByClassName && speedUpMatch[2]) {
                                ...
                                for (var sui = 0, sulen = suElems.length; sui < sulen; ++sui) {
                                    suIt = suElems[sui]; ((' ' + suIt.className + ' ').indexOf(suCN) > -1) && suResBuff.push(suIt);
                                }
    ...
    QZFL.object.each(['onClick', 'onMouseDown', 'onMouseUp', 'onMouseOver', 'onMouseMove', 'onMouseOut', 'onFocus', 'onBlur', 'onKeyDown', 'onKeyPress', 'onKeyUp'], 
    function(name, index) {
    ...
    QZFL.effect = {
        ...
        mode: [],
        init: function() {
            var classArray = [['webkit', 'WebkitTransition'], ['firefox', 'MozTransition'], ['opera', 'OTransition'], ['ie', 'msTransition']],
            ua = QZFL.userAgent,
            agent = '',
            cName = '';
        ...
        run: function(elem, prop, opts) {
            ...
            if (o.off) {
                ...
                function() {
                    o._tweenArray[tid] = t; (new QZFL.cssTransfrom(elem, fprop, opts)).firecss();
                }: function() {
                    o._tweenArray[tid] = t;
                };
        ...
        stop: function(elem) {
            ...
            o;
            elem = QZFL.dom.get(elem);
            if (webkit) { (o = elem._transition) && o.stop();
            } else {
                var tid = elem._tid,
        ...
        slideDown: function(elem, opts) {
            ...
            complete;
            elem = d.get(elem);
            attrs = o._checkVerticalStyle(elem, {
                status: 'down'
            });
            opts = opts || {};
            if (attrs) {
    ...
    QZFL.tweenMaker.prototype = {
        ...
        _playTimer: function(time) {
            var _end = false,
            o = this,
            time = (new Date().getTime() - o._startTime);
            if (time > o.duration) {
                time = o.duration;
                _end = true;
            }
    ...
    QZFL.cssTransfrom.prototype = {
        init: function() {
            var map = [['webkit', '-webkit-transition', 'WebkitTransition', 'webkitTransitionEnd', 'WebkitTransform'], ['firefox', '-moz-transition', 'MozTransition', 'transitionend', 'MozTransform'], ['opera', '-o-transition', 'OTransition', 'oTransitionEnd', 'OTransform'], ['ie', '-ms-transition', 'msTransition', 'MSTransitionEnd', 'msTransform']],
            tiClassPrefix,
            tiStyleName,
            tiEvtName,
            tfStyleName,
        ...
        _toHyphen: function(prop) {
            prop = prop.replace(/([A-Z]?)([a-z]+)([A-Z]?)/g, 
    ...
    QZFL.pingSender = function(url, t, opts) {
        ...
        })(img); (typeof(opts.timeout) == 'function') && setTimeout(function() {
    ...
    QZFL.dragdrop = {
        path: "./js/dragdrop.js?max_age=864001",
        dragdropPool: {},
        count: 0,
    ...
    QZFL.widget.msgbox = {
        ...
        hide: function(timeout) {
            QZFL.imports(QZFL.widget.msgbox.path, 
            function() {
    ...
    QZFL.dialog = {
        cssPath: "css/qz_dialog.css",
        currentCssPath: '',
        path: "./js/dialog.js?max_age=864010",
        count: 0,
        instances: {},
        BUTTON_TYPE: {
            Disabled: -1,
    ...
    QZFL.widget.Confirm.cssPath = "css/confirm_by_dialog.css";
    QZFL.widget.Confirm.path = "./js/confirm_base.js";
    ...
    QZFL.maskLayout = (function() {
        var masker = null,
        count = 0,
        qml = function(zi, doc, opts) {++count;
            if (masker) {
                return count;
            }
            zi = zi || 5000;
    ...
    QZFL.widget.bubble = {
        show: function(target, title, msg, opts) {
            ...
            opts.id = bid;
            QZFL.imports(QZFL.widget.bubble.path, 
            function() {
    ...
    QZFL.widget.tips = {
        ...
        show: function(html, aim, opts) {
            ...
            opts.id = bid;
            QZFL.imports(QZFL.widget.tips.path, 
            function() {
    ...
    QZFL.object.map(QZFL.lang || {}); (function(w) {
        w.ua = w.ua || QZFL.userAgent;
        w.$e = QZFL.element.get; ! w.$ && (w.$ = QZFL.dom.get);
    ...
    QZFL.pluginsDefine.networkChectLibPath = 'http://' + QZFL.config.resourceDomain + '/qzone/v6/troubleshooter/network_check_plugin_lib.js'; (function() {
        var t = QZONE.FormSender;
        if (t && t.pluginsPool) {
                    ...
                    if (d && (d = d[0])) {
                        if (typeof d.code == 'undefined') {
                            ...
                            if (th instanceof q.JSONGetter) {
                                if (th.reportRate) { (th.reportRate == 1 || Math.random() < 1 / th.reportRate) && collector.push([domain, url, 1, d.subcode || 1, +th.endTime - th.startTime, th.reportRate || 2000]);
                                }
                                else {
                            ...
                            if (th instanceof q.FormSender) {
                                if (th.reportRate) { (th.reportRate == 1 || Math.random() < 1 / th.reportRate) && collector.push([domain, url, 1, d.subcode || 1, +th.endTime - th.startTime, th.reportRate || 100]);
                                }
                                else {
