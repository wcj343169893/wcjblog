...
public class hbqnb {
	...
	static Logger logger = Logger.getLogger(hbqnb.class);
	final static String nspName = "hbqnb";
	// final static String imgPath = "F:/gw/updatesoft/报纸采集/";
	...
	public static String[] getNewsDetailList(String content, String titleNm) {
		...
		while (m.find()) {
			...
			tmp[0] = m.group(1);
			/*
			 * if(tmp[0].contains("石市新华区公车要全贴")){ logger.info("---------"); }
			 */
	...
	public static void cj(String rq) {
		...
		try {
			...
			for (String[] strings : ContentList) {
				...
				dt
						.writefile(strings[2].replaceAll("/", "-").replaceAll(":", "-"), strings[3],
