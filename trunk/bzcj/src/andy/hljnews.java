 ...
 */
public class hljnews {

	static Logger logger = Logger.getLogger(hljnews.class);
	...
	public static void cj(String rq) throws Exception {
		...
		try {
			...
			for (String[] strings : ContentList) {
				...
				dt
						.writefile(strings[2].replaceAll("/", "-"), strings[3],
								"黑龙江新闻报", "黑龙江新闻报(" + ycjrq.replaceAll("/", "-") + ")", replaceChar(strings[1].replaceAll("·", "-")),
								strings[0]);// 生成txt
				if (strings[4] != null && !"".equals(strings[4])
					...
					for (int i = 0; i < imgFiles.length; i++) {
						...
						dt.wgetpic(imgFiles[i], imgFileNm.replaceAll("/", "-"),
								imgPath + "黑龙江新闻报/黑龙江新闻报(" + ycjrq.replaceAll("/", "-") + ")/"
										+ replaceChar(strings[1]).replaceAll("·", "-"));
					}
				}
			...
			for (String[] strings : bmPdfList) {
				try {
					// 保存版面的所有PDF图片
					String dirPath = imgPath + "黑龙江新闻报/黑龙江新闻报(" + ycjrq.replaceAll("/", "-")
							+ ")/版面图片/";
	...
	static String replaceChar(String fname) {
		...
		try {
			 newFileNm = fname.replace("?", "").replace("“", "").replace("”", "");
