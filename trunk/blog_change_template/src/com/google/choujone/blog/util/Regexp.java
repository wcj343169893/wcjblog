 ...
 * 在这里，我将这个类定义成了一个单例， （本来只有static 方法是不new去对象的，也就用不到单例了）<br/>
 * 是因为有一个 regexpHash(HashMap实例)，为了可以动态 添加正规表达式（不知道是不是想的太多了:) ）。
 * 
 * <pre>
...
public final class Regexp {
	...
	public static Set getBetweenSeparatorStr(final String originStr,
		...
		if (originStr == null || "".equals(originStr.trim())) {
			return variableSet;
		}
	...
	public static void main(String a[]) {
		 ...
		 * matcher = new Perl5Matcher(); MatchResult matchResult =
		 * matcher.getMatch(); String email = "wuzhi2000@hotmail.com.cn"; String
		 * email_regexp = "^[\\w-]+(\\.[\\w-]+)*@[\\w-]+(\\.[\\w-]+)+$"; String
		 * url = "http://www"; String url_regexp =
		 ...
		 * "forum/head_icons/anoymous20050428125334.jpg"; System.out
		...
		//
		// Pattern p1 = compiler.compile(regexpForFontTag,
		// Perl5Compiler.CASE_INSENSITIVE_MASK);
		// Pattern p2 = compiler.compile(regexpForFontAttrib,
		// Perl5Compiler.CASE_INSENSITIVE_MASK);
		...
		try {
			...
			Pattern p1 = compiler.compile(http_regexp,
					Perl5Compiler.CASE_INSENSITIVE_MASK);
