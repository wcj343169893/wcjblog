...
<div class="context_right_all">
	<div class="tabs_150_context">
    	...
    	<div class="tabs_150_context_center">
    		<h1 class="font1">科目分析</h1><br/>
    		<c:if test="${rid==5||rid==1}">
    		   	<a href="scores_analysis-2.html">班级名次</a>&nbsp;&nbsp;&nbsp;    
    		</c:if>		
    			<a href="scores_analysis-0.html">学生总分</a>&nbsp;&nbsp;&nbsp;
    			<font class="font3">学生各科成绩</font>		
    	</div>
    	<div class="tabs_150_context_right"></div>
    </div>
    <!-- 选择学生及统计方式 -->
    <div class="list_items font1 bg" style="text-align:left">请选择学生姓名及统计方式</div> 
		       	<c:choose>
		    ...
		    <div class="list_bottom_query">
				...
				<c:choose>
		    		<c:when test="${rid==5||rid==1||rid==2}">
							学生姓名：<select name="uid" id="uid" onchange="changeSubjectByUser()">
											<option value="0">请选择</option>
									 </select>    
							学科：<select name="sid" id="sid">
											<option value="0">请选择</option>
									 </select>  		
		    		</c:when>
		    		<c:otherwise>
		...
		<div class="list_bottom_query">
			...
			<input type="button" onclick="subjectAnalysis()" value="开始查询" id="subjectAnalysisSearch"/>  		
		</div>
    <!-- 列名  -->   
